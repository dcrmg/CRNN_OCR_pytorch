...
parser.add_argument('--trainroot', help='path to dataset',default='../lmdb/train')
parser.add_argument('--valroot', help='path to dataset',default='../lmdb/val')
...
parser.add_argument('--lr', type=float, default=0.0005, help='learning rate for Critic, default=0.00005')
...
for epoch in range(opt.niter):
    ...
    while i < len(train_loader):
        ...
        if i % opt.valInterval == 0 and i !=0:
            ...
            #print('Test loss: %f, accuray: %f' % (testLoss, accuracy))
            print("epoch:{},step:{},Test loss:{},accuracy:{},train loss:{}".format(epoch,i,testLoss,accuracy,loss_avg.val()))
            loss_avg.reset()
        # do checkpointing
        num +=1
        ...
        if lasttestLoss >testLoss:
            ...
            #delete(opt.experiment)##保存当前模型，删除上一个保存的模型
            torch.save(crnn.state_dict(), '{}/netCRNN_{}.pth'.format(opt.experiment,accuracy))
            if os.path.exists('{}/netCRNN_{}.pth'.format(opt.experiment,test_acc)):
                os.remove('{}/netCRNN_{}.pth'.format(opt.experiment,test_acc))
            test_acc = accuracy
            numLoss = 0
        else:
            numLoss+=1
